...
class Project(object):
    ...
    def template(self, identifier, name=False):
        """
        ...
        """
        if name:
            matches = [template for template in self.templates if template.name == identifier]
        else:
            matches = [template for template in self.templates if template.slug == identifier]
        if len(matches) == 1:
            return matches[0]
        else:
            raise TemplateSlugError('{} is not a valid template slug.'.format(identifier))
